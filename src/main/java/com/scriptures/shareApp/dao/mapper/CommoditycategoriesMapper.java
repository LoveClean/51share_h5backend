...
public interface CommoditycategoriesMapper {
    ...
    @Select("select * from tb_commoditycategories where del_flag = 0")
    List<Commoditycategories> selectAll();

    @Select("select count(1) from tb_commoditycategories where id = #{id} and del_flag = 0")
    int checkById(String id);

    @Select("select count(1) from tb_commoditycategories where categoriesname = #{categoriesname} and del_flag = 0")
    int checkName(@Param("categoriesname") String categoriesname);

    @Select("select count(1) from tb_commoditycategories where categoriesname = #{categoriesname} and id != #{id} and del_flag = 0")
    int checkUptName(@Param("categoriesname") String categoriesname, @Param("id") String id);

    @Select("select categoriesname from tb_commoditycategories WHERE id = #{id} and del_flag = 0")
    String selectNameById(@Param("id") String id);

    @Select("select count(1) from tb_commoditycategories where orderby = #{orderby} and del_flag = 0")
    int checkOrderby(Integer orderby);

    @Select("select count(1) from tb_commoditycategories where orderby = #{orderby} and id != #{id} and del_flag = 0")
